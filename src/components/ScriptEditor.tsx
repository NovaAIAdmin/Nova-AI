  ...
  return (
    <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-slate-700">
      ...
      <Textarea
        ...
        className="min-h-[200px] bg-slate-900 border-slate-700 focus:ring-amber-500 focus:border-amber-500"
      />
        ...
        <div className="mt-4">
          ...
          <div className="space-y-2">
            {aiSuggestions.map((suggestion, index) => (
              <div 
                key={index} 
                className="p-3 bg-slate-900/50 border border-slate-700 rounded-lg flex justify-between items-start"
              >
                <div className="text-slate-300">{suggestion}</div>
                <Button 
                  size="sm"
        ...
        <div className="mt-6">
          ...
          <div className="space-y-3 max-h-60 overflow-y-auto">
            {segments.map((segment, index) => (
              <div 
                key={segment.id} 
                className="p-3 bg-slate-900/30 border border-slate-700 rounded-lg"
              >
                <div className="text-xs text-slate-500 mb-1">
                  Segment {index + 1} • {Math.round(segment.duration / 1000)}s
                </div>
                <div className="text-slate-300">{segment.text}</div>
              </div>
            ))}
          </div>
        </div>
